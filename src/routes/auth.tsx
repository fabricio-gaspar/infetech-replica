...
const PROVIDERS_URL = "https://supabase.com/dashboard/project/bxwyeosqffbbkwqpyttd/auth/providers";
...
function AuthPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      if (data.session && redirect?.startsWith("/")) navigate({ to: redirect });
    });
  }, [navigate]);
  ...
  const handleGoogle = async () => {
    ...
    if (error) {
      ...
      const msg = /not enabled|unsupported provider/i.test(error.message)
        ? "O provedor Google ainda não foi habilitado no painel do Supabase. Um administrador precisa ativá-lo (com Client ID e Client Secret do Google Cloud) para que este botão funcione."
        : error.message;
      setGoogleError(msg);
            ...
            <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
              <div className="flex gap-2">
                ...
                <div className="space-y-2">
                  <p>{googleError}</p>
                  <a href={PROVIDERS_URL} target="_blank" rel="noreferrer" className="font-semibold underline underline-offset-2">
                    Abrir provedores no Supabase →
                  </a>
                </div>
              </div>
            </div>
